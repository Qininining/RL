"""Entry point to evaluate a trained Q-learning agent on the snake game environment."""
...
from snake_game.config import ProjectConfig, parse_config_from_dict
from snake_game.training.q_learning import create_env

PROJECT_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "configs" / "snake.yaml"
...
def evaluate(cfg: ProjectConfig, model_path: Path, episodes: int | None, render_override: bool, delay: float) -> None:
    ...
    for episode in range(num_episodes):
        ...
        print(f"Episode {episode + 1} finished. Reward: {episode_reward:.2f}, Food Eaten: {food_eaten}")
    ...
    print(f"Success Rate (at least 1 food): {success_rate:.2%}")
