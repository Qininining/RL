"""Entry point to train a Q-learning agent on the snake game environment."""
...
from snake_game.training.q_learning import train_q_learning
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Q-learning agent for the SnakeEnv.")
    parser.add_argument(
        "--config",
        type=str,
...
def apply_overrides(config: ProjectConfig, args: argparse.Namespace) -> ProjectConfig:
    ...
    if args.render:
        config.training.render_mode = "human"
    return config
